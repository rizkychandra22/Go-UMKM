    ...
    >
      <CarouselContent className="h-full ml-0">
        {banners.map((banner) => (
          <CarouselItem
            key={banner.id}
            className="pl-0 min-h-[200px] sm:min-h-[300px] h-full relative"
          >
            <img
              src={banner.imgUrl}
              alt={banner.title}
              className="absolute inset-0 w-full h-full object-cover object-center opacity-40 dark:opacity-30"
            />
            <div
              className={`absolute inset-0 opacity-80 ${banner.bgColor} mix-blend-multiply`}
            />

            <div className="absolute inset-0 flex flex-col justify-center px-6 sm:px-16 md:px-24 text-white">
              <h2 className="text-xl sm:text-4xl md:text-5xl font-extrabold tracking-tight mb-1 sm:mb-4 fade-in-up">
                {banner.title}
              </h2>
              <p className="text-xs sm:text-lg md:text-xl font-medium opacity-90 mb-4 sm:mb-8 fade-in-up-delay max-w-xl">
                {banner.subtitle}
              </p>
