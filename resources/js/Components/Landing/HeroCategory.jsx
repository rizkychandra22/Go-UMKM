...
export default function CategoryProduct({ categories }) {
  ...
  const getStyle = (slug) => {
    const styles = {
      'kuliner-lokal': {
        icon: HandPlatter,
        tone: 'from-amber-50 to-amber-100 dark:from-amber-900/40 dark:to-amber-950/60',
      },
      'trending-fashion': {
        icon: Shirt,
        tone: 'from-pink-50 to-rose-100 dark:from-rose-900/40 dark:to-rose-950/60',
      },
      'rumah-dekor': {
        icon: House,
        tone: 'from-sky-50 to-sky-100 dark:from-sky-900/40 dark:to-sky-950/60',
      },
      'kebutuhan-harian': {
        icon: ShoppingBasket,
        tone: 'from-emerald-50 to-emerald-100 dark:from-emerald-900/40 dark:to-emerald-950/60',
      },
    };
    ...
    return (
      styles[slug] ?? {
        ...
        tone: 'from-slate-50 to-slate-200 dark:from-slate-800 dark:to-slate-900',
      }
    );
  };
        ...
        {isHome && (
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Pilih sesuai kebutuhan: kuliner harian sampai hampers premium.
            </p>
          </div>
        )}
          ...
          return (
            <Link href={href} key={item.id}>
              ...
              >
                ...
                <p className="text-[11px] sm:text-xs leading-relaxed text-muted-foreground line-clamp-2">
                  {item.description}
                </p>
              </div>
            </Link>
