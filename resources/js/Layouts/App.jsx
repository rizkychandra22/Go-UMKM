...
export default function LayoutApp({ pageTitle, children }) {
    ...
    if (isDashboard) {
        ...
        navItems.push({ label: 'Keluar', href: route('logout'), type: 'logout', badge: true, icon: LogOut });
    } else {
        navItems = [
            { label: 'Beranda', href: route('home'), type: 'link', variant: 'home', icon: House },
        ];
        ...
        if (auth.user) {
            ...
            navItems.push({ label: 'Masuk', href: route('login'), type: 'link', badge: true, icon: LogIn });
        }
    }
                ...
                <header className="glass-panel sticky top-4 z-50 px-4 py-3 sm:px-5">
                    <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Go-Umkm</p>
                        ...
                        <nav className="hidden items-center gap-1 sm:flex">
                            {navItems.map((item, idx) => {
                                ...
                                const baseClass = "group relative inline-flex items-center gap-2 px-4 py-2 text-sm font-bold transition-all duration-300 rounded-xl overflow-hidden";
                                const variantClass = item.badge
                                    ...
                                    : isActive 
                                        ...
                                        : item.variant === 'home'
                                            ? 'bg-linear-to-br from-teal-500/10 to-orange-500/10 border border-teal-200/50 text-teal-700 hover:from-teal-500/20 hover:to-orange-500/20 hover:border-teal-300'
                                            : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900';

                                if (item.type === 'logout') {
                                    return <button key={idx} onClick={() => router.post(item.href)} className={`${baseClass} ${variantClass}`}>{Icon && <Icon className="size-4 shrink-0" />} {item.label}</button>;
                                }
                    ...
                    {isMenuOpen && (
                        <nav className="mt-4 flex flex-col gap-2 border-t border-slate-100 pt-4 sm:hidden fade-in-up">
                            {navItems.map((item, idx) => {
                                ...
                                // Penyesuaian class active mobile agar lebih kontras
                                const variantClass = item.badge
                                    ? 'bg-slate-900 text-white hover:bg-slate-800 ml-1 shadow-lg shadow-slate-200'
                                    : isActive 
                                        ? 'bg-linear-to-r from-teal-600 to-teal-500 text-white shadow-lg shadow-teal-200 -translate-y-0.5'
                                ...
                                if (item.type === 'logout') {
                                    return (
                                        <button key={idx} onClick={() => { setIsMenuOpen(false); router.post(item.href); }} className={`${baseClass} ${variantClass}`}>
