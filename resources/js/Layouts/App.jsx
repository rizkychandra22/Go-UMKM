...
export default function LayoutApp({ pageTitle, children, loading = false }) {
  ...
  const dashboardMenus = [
    ...
    },
    {
      label: 'Produk',
      href: isSeller
        ? route('dashboardSeller', { tab: 'produk' })
        : route('dashboardCustomer', { tab: 'produk' }),
      icon: Package,
      section: 'MANAGEMENT',
    },
    isSeller
      ? {
          label: 'Penjualan',
          href: route('dashboardSeller', { tab: 'penjualan' }),
          icon: ClipboardList,
          section: 'MANAGEMENT',
        }
      : {
          label: 'Keranjang',
          href: route('dashboardCustomer', { tab: 'keranjang' }),
          icon: ShoppingCart,
          section: 'MANAGEMENT',
        },
    {
      label: 'Pesanan',
      href: isSeller
        ? route('dashboardSeller', { tab: 'pesanan' })
        : route('dashboardCustomer', { tab: 'pesanan' }),
      icon: ClipboardList,
      section: 'MANAGEMENT',
    },
    {
  ...
  if (isDashboard) {
    ...
    return (
      <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
        {/* Desktop Sidebar */}
        <aside
          className={`fixed left-0 top-0 z-40 hidden h-screen w-[260px] border-r border-border bg-sidebar text-sidebar-foreground transition-all duration-300 lg:block ${
        ...
        >
          <div className="flex h-20 items-center gap-3 border-b border-border px-7">
            ...
            <div>
              ...
              <h1 className="truncate text-base font-extrabold leading-none text-sidebar-foreground">
                {pageTitle}
              </h1>
            </div>
          </div>
          ...
          <nav className="h-[calc(100vh-160px)] overflow-y-auto px-5 py-6">
            {sections.map((section) => (
              <div key={section} className="mb-7 last:mb-0">
                <p className="mb-3 px-3 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">
                  {section}
                </p>
        ...
        {/* Dashboard Main Area */}
        <main
          className={`transition-all duration-300 ${isSidebarCollapsed ? 'lg:pl-0' : 'lg:pl-[260px]'}`}
        >
          <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-border bg-background/95 px-5 backdrop-blur-sm lg:px-8 transition-colors duration-300">
            <div className="flex items-center gap-4">
              ...
              >
                <PanelLeft
                  className={`size-5 transition-transform duration-300 ${isSidebarCollapsed ? 'rotate-180' : ''}`}
                />
              </Button>
            ...
            <div className="flex items-center gap-2">
              <Link
                href={route('home')}
                className="hidden items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-muted-foreground transition hover:bg-accent hover:text-foreground sm:inline-flex"
              >
    ...
    <div className="relative min-h-screen flex flex-col bg-background text-foreground transition-colors duration-300">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur-sm shadow-sm transition-colors">
        <div className="mx-auto flex w-full max-w-[1280px] items-center gap-3 sm:gap-6 px-4 py-3">
            ...
            {navItems.map((item, idx) => {
              ...
              return (
                <Link
                  key={idx}
                  href={item.href}
                  className={`transition-colors ${active ? 'text-teal-600 font-bold dark:text-teal-400' : 'text-slate-600 hover:text-teal-600 dark:text-slate-400 dark:hover:text-teal-400'}`}
                >
                  {item.label}
                </Link>
              );
          ...
          {/* Header Actions */}
          <div
            className="flex shrink-0 items-center gap-2 sm:gap-4 ml-auto"
            ref={profileRef}
          >
            {/* Cart */}
            <Link
              href={
                auth?.user
                  ? isSeller
                    ? route('dashboardSeller', { tab: 'pesanan' })
                    : route('dashboardCustomer', { tab: 'keranjang' })
                  : route('login')
              }
            ...
            >
              ...
              <span className="absolute right-0 top-0 flex size-4 items-center justify-center rounded-full bg-orange-500 text-[10px] font-bold text-white border-2 border-white dark:border-slate-950">
                3
              </span>
            </Link>
            ...
            {auth?.user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 rounded-xl p-1 pr-3 border border-transparent hover:bg-slate-100 dark:hover:bg-slate-800 transition"
                >
                  ...
                  <span className="hidden md:block text-sm font-semibold truncate max-w-[100px] text-slate-700 dark:text-slate-200">
                    {auth?.user?.name}
                  </span>
                </button>
                {isProfileOpen && (
                  <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-xl border border-slate-200 bg-white p-2 shadow-lg dark:border-slate-700 dark:bg-slate-900 fade-in-up">
                    ...
                    <Link
                      ...
                      className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
                    >
                      <Settings className="size-4 text-slate-500" /> Profile
                      User
                    </Link>
                    {isSeller && (
                      <Link
                        ...
                        className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
                      >
                        <Store className="size-4 text-slate-500" /> Profile
                        Bisnis
                      </Link>
                    )}
                    <button
                      onClick={() => router.post(route('logout'))}
                      className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30"
                    >
              ...
              <div className="hidden sm:flex items-center gap-2">
                <Link
                  href={route('login')}
                  className="inline-flex items-center justify-center rounded-xl border border-teal-600 px-4 py-2 text-sm font-semibold text-teal-600 transition hover:bg-teal-50 dark:border-teal-500 dark:text-teal-400 dark:hover:bg-teal-950/30"
                >
                  Masuk
                </Link>
                <Link
                  href={route('register')}
                  className="inline-flex items-center justify-center rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700 focus-visible:outline-teal-600"
                >
                  Daftar
                </Link>
              </div>
            ...
            <button
              ...
              className="md:hidden rounded-xl p-2 text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              {isMenuOpen ? (
                <X className="size-6" />
              ) : (
            ...
            <div className="flex flex-col gap-1">
              {navItems.map((item, idx) => {
                ...
                return (
                  <Link
                    key={idx}
                    href={item.href}
                    className={`flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-semibold transition-colors ${active ? 'bg-teal-50 text-teal-700 dark:bg-teal-900/30 dark:text-teal-400' : 'text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-900'}`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {item.icon && (
                      <item.icon
                        className={`size-5 ${active ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400'}`}
                      />
                    )}{' '}
                    {item.label}
                  </Link>
              ...
              {!auth?.user && (
                <div className="mt-2 flex gap-3 px-1">
                  <Link
                    href={route('login')}
                    className="flex-1 inline-flex items-center justify-center rounded-xl border border-teal-600 px-4 py-2.5 text-sm font-semibold text-teal-600 transition hover:bg-teal-50 dark:border-teal-500 dark:text-teal-400 dark:hover:bg-teal-950/30"
                  >
                    Masuk
                  </Link>
                  <Link
                    href={route('register')}
                    className="flex-1 inline-flex items-center justify-center rounded-xl bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700 focus-visible:outline-teal-600"
                  >
                    Daftar
                  </Link>
                </div>
        ...
        <div className="mx-auto w-full max-w-[1280px] px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div className="lg:col-span-1">
              ...
              >
                ...
                <h2 className="text-xl font-extrabold tracking-tight text-teal-600 dark:text-teal-400">
                  Tokoku
                </h2>
              </Link>
              <p className="text-sm text-slate-600 dark:text-slate-400 max-w-xs leading-relaxed">
                Marketplace lokal yang mendukung UMKM di seluruh Indonesia.
                Belanja bijak, dukung usaha mikro, kecil, dan menengah untuk
          ...
          <div className="mt-12 flex flex-col md:flex-row items-center justify-between border-t border-slate-200 dark:border-slate-800 pt-8">
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Â© 2026 Tokoku. Seluruh hak cipta dilindungi.
            </p>
