...
export default function DashboardCustomer({ categories = [] }) {
    ...
    // 1. State Object kuantitas produk
    const [quantities, setQuantities] = useState({});

    // 2. Fungsi handler penambah/pengurang qty
        ...
        <>
            ...
            <LayoutApp pageTitle="Dashboard Seller">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
                        <div className="bg-white/50 p-4 rounded-2xl border border-slate-200 hover:shadow-sm transition-all">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-teal-50 rounded-xl text-teal-600"><ShoppingCart className="size-5" /></div>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Keranjang</p>
                            </div>
                        ...
                        <div className="bg-white/50 p-4 rounded-2xl border border-slate-200 hover:shadow-sm transition-all">
                            <div className="flex items-center gap-3">
                                <div className="p-2 bg-sky-50 rounded-xl text-sky-600"><ClipboardCheck className="size-5" /></div>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Riwayat Belanja</p>
                            </div>
                            <p className="mt-2 text-2xl font-black text-slate-900">{paymentHistory.length}</p>
                        </div>
                    </div>
                ...
                <section className="glass-panel fade-in-up p-6 sm:p-8 border-t-4 border-t-emerald-400 mt-6 flex flex-col">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-600">Daftar produk yang telah ditambahkan ke keranjang.</p>
                        </div>
                        ...
                        <div className="grid gap-4 lg:grid-cols-2">
                            {filteredProducts.length > 0 ? (
                                filteredProducts.map((product, index) => {
                                    // PENGAMAN: Jika product.id kosong/ganda, paksa pakai index agar tetap unik per baris item
                                    const itemKey = product.id || `idx-${index}`;
                                    const currentQty = quantities[itemKey] || 1;

                                    return (
                                        <div key={`product-${itemKey}`} className="flex items-center gap-4 rounded-2xl border border-slate-300 bg-white p-4 transition-all hover:border-teal-300 hover:shadow-md">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="mt-3 flex items-center justify-between gap-2 flex-wrap">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <Link className="inline-flex items-center gap-1.5 rounded-xl bg-teal-50 px-3 py-1.5 text-xs font-bold text-teal-700 hover:bg-teal-600 border border-teal-200 hover:text-white hover:border-teal-600 transition-all">
                                                             <ShoppingCart className="size-3" /> Checkout
                                                        </Link>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <section className="glass-panel fade-in-up-delay p-6 sm:p-8 border-t-4 border-t-emerald-400 mt-6 flex flex-col">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            <h3 className="text-2xl font-extrabold text-slate-900">Pesanan Aktif</h3>
                            <p className="mt-1 text-sm text-slate-600">Daftar pesanan yang sedang diproses.</p>
                        </div>
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="mt-2 flex items-center justify-between">
                                                ...
                                                <Link className="inline-flex items-center gap-1 text-xs font-bold text-teal-700 hover:gap-2 transition-all">
                                                    Lacak <ArrowRight className="size-3" />
                                                </Link>
                                            </div>
                                        </div>
                                    </div>
                ...
                <section className="glass-panel fade-in-up-delay p-6 sm:p-8 border-t-4 border-t-emerald-400 mt-6 flex flex-col">
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="mt-1 text-sm text-slate-600">Riwayat pembayaran pesanan Anda</p>
                        </div>
