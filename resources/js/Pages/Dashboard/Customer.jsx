...
export default function DashboardCustomer({ categories = [] }) {
  ...
  const currentTab = (() => {
    try {
      ...
      return ['produk', 'keranjang', 'pesanan'].includes(activeTab)
        ? activeTab
        : 'all';
    } catch {
      return 'all';
  ...
  const showKeranjangSection =
    currentTab === 'all' || currentTab === 'keranjang';
          ...
          <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-6 sm:mt-8">
            <div className="bg-white/50 p-3 sm:p-4 rounded-2xl border border-slate-200 hover:shadow-sm transition-all dark:bg-white/10 dark:bg-slate-900/50">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="bg-teal-50 rounded-xl text-teal-600 dark:text-teal-400 dark:bg-teal-950/50">
                  <ShoppingCart className="size-4 sm:size-5" />
                </div>
                <p className="text-[9px] sm:text-[10px] font-black text-slate-600 uppercase dark:text-slate-400 tracking-widest">
                  Keranjang
                </p>
              </div>
            ...
            <div className="bg-white/50 p-3 sm:p-4 rounded-2xl border border-slate-200 hover:shadow-sm transition-all dark:bg-white/10 dark:bg-slate-900/50">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="bg-orange-50 rounded-xl text-orange-600 dark:text-teal-400 dark:bg-teal-950/50">
            ...
            <div className="bg-white/50 p-3 sm:p-4 rounded-2xl border border-slate-200 hover:shadow-sm transition-all dark:bg-white/10 dark:bg-slate-900/50">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="bg-sky-50 rounded-xl text-sky-600 dark:text-sky-400 dark:bg-sky-950/50">
          ...
          <section className="glass-panel fade-in-up p-5 sm:p-8 border-t-4 border-t-emerald-400 mt-6 flex flex-col dark:bg-slate-900/40 dark:border-x-slate-800 dark:border-b-slate-800">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <h3 className="text-lg sm:text-2xl font-extrabold text-slate-900 dark:text-slate-100">
                  Keranjang
                </h3>
                <p className="mt-1 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                  Daftar produk yang ada dikeranjang Anda.
                </p>
              </div>
                ...
                {filteredProducts.length > 0 ? (
                  filteredProducts.map((product, index) => {
                    ...
                    return (
                      <div
                        key={`product-${itemKey}`}
                        className="flex items-center gap-3 sm:gap-4 rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 transition-all hover:border-teal-300 hover:shadow-md dark:bg-slate-800 dark:border-slate-600 dark:hover:border-teal-300"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <h4 className="text-sm font-bold text-slate-900 truncate mt-1 dark:text-slate-100">
                            {product.name}
                          </h4>
                          <div className="mt-1 flex items-center justify-between gap-2 flex-wrap">
                            ...
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-[11px] font-extrabold px-2 py-0.5 rounded-md ${product.stock <= 5 ? 'bg-rose-50 text-rose-600' : 'bg-slate-50 text-slate-600'}`}
                              >
                                Stok: {product.stock}
                              </span>
                  ...
                  {filteredOrders.length > 0 ? (
                    filteredOrders.map((order, index) => (
                      <div
                        ...
                        className="flex items-start gap-3 sm:gap-4 rounded-2xl border border-slate-200 bg-white p-3 sm:p-4 transition-all hover:border-teal-300 hover:shadow-md dark:bg-slate-800 dark:border-slate-600 dark:hover:border-teal-300"
                      >
                        <div className="size-16 sm:size-20 flex-none overflow-hidden rounded-xl bg-slate-50 border border-slate-100">
                          <img
                            src={order.image}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-start justify-between gap-2 flex-wrap">
                            <p className="text-xs sm:text-sm font-black text-slate-900 dark:text-slate-100">
                              {order.amount}
                              <span className="ml-1 sm:ml-2 text-[10px] sm:text-xs font-medium text-slate-500">
                                Â· {order.qty} pcs
                              </span>
                            </p>
                          </div>
            ...
            <section className="glass-panel fade-in-up p-5 sm:p-8 border-t-4 border-t-emerald-400 mt-6 flex flex-col dark:bg-slate-900/40 dark:border-x-slate-800 dark:border-b-slate-800">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                    Riwayat pembayaran pesanan Anda
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    ...
                    <tbody>
                      {filteredPayments.length > 0 ? (
                        filteredPayments.map((pay) => (
                          <tr
                            key={pay.id}
                            className="group bg-white border border-slate-200 hover:border-teal-300 transition-all dark:bg-slate-800 dark:border-slate-600 dark:hover:border-teal-300"
                          >
                            ...
                            <td className="px-4 py-4 border-y border-slate-200 group-hover:border-teal-300 text-xs text-slate-500 dark:text-slate-400">
                              {pay.date}
                            </td>
                            <td className="px-4 py-4 border-y border-slate-200 group-hover:border-teal-300 text-xs font-bold text-slate-900 dark:text-slate-100">
                              {pay.product}
                            </td>
                            ...
                            <td className="px-4 py-4 border-y border-slate-200 group-hover:border-teal-300 text-sm font-black text-slate-900 dark:text-slate-100 dark:bg-slate-800">
                              {pay.amount}
                            </td>
                ...
                <div className="space-y-3 sm:hidden">
                  {filteredPayments.length > 0 ? (
                    filteredPayments.map((pay) => (
                      <div
                        key={`mobile-${pay.id}`}
                        className="rounded-2xl border border-slate-200 bg-white p-4 space-y-2 hover:border-teal-300 transition-all"
                      >
