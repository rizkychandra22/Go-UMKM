        ...
        <LayoutApp pageTitle="Pengaturan Profil">
            ...
            <div className="w-full">
                <section className={`glass-panel fade-in-up p-6 sm:p-8 border-t-4 ${auth.user.role === 'seller' ? 'border-t-emerald-400' : 'border-t-indigo-400'}`}>
                    <div className="mb-8">
                        <h2 className="text-2xl font-black text-slate-900">Informasi Pribadi & Keamanan</h2>
                        <p className="text-sm text-slate-500 font-medium mt-1">Kelola data diri, alamat, dan kata sandi Anda dalam satu tempat.</p>
                    </div>
                    ...
                    {status === 'profile-updated' && (
                        <div className="mb-6 flex items-center gap-3 rounded-2xl bg-teal-100 p-4 text-teal-700 border border-teal-150 animate-in fade-in zoom-in duration-300">
                            ...
                            <p className="text-sm font-bold">Profil dan pengaturan berhasil diperbarui!</p>
                        </div>
                    )}
                    ...
                    <form onSubmit={submit} className="space-y-8">
                        ...
                        <div className="flex flex-col items-center justify-center gap-4 sm:flex-row sm:justify-start bg-slate-50/50 p-4 rounded-3xl border border-slate-100">
                            ...
                            <div className="text-center sm:text-left">
                                ...
                                {errors.image && <p className="text-red-500 text-[10px] font-bold mt-1 uppercase">{errors.image}</p>}
                            </div>
                        </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">Data Identitas</h3>
                                
                                <div className="space-y-1.5">
                                    <label className="ml-1 text-xs font-bold text-slate-600">Nama Lengkap</label>
                                    <div className="relative">
                                        <User className="absolute left-3 top-3 size-4 text-slate-400" />
                                        <input type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} className="w-full rounded-2xl border-slate-200 bg-white p-2.5 pl-10 text-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" />
                                    </div>
                                    {errors.name && <p className="text-red-500 text-[10px] font-bold ml-1">{errors.name}</p>}
                                </div>

                                <div className="space-y-1.5">
                                    <label className="ml-1 text-xs font-bold text-slate-600">Nomor HP / WhatsApp</label>
                                    <div className="relative">
                                        <Phone className="absolute left-3 top-3 size-4 text-slate-400" />
                                        <input type="text" value={data.phone} onChange={(e) => setData('phone', e.target.value)} className="w-full rounded-2xl border-slate-200 bg-white p-2.5 pl-10 text-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" />
                                    </div>
                                    {errors.phone && <p className="text-red-500 text-[10px] font-bold ml-1">{errors.phone}</p>}
                                </div>

                                <div className="space-y-1.5">
                                    <label className="ml-1 text-xs font-bold text-slate-600">Alamat Email</label>
                                    <div className="relative">
                                        <Mail className="absolute left-3 top-3 size-4 text-slate-400" />
                                        <input type="email" value={data.email} onChange={(e) => setData('email', e.target.value)} className="w-full rounded-2xl border-slate-200 bg-white p-2.5 pl-10 text-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" />
                                    </div>
                            ...
                            <div className="space-y-4">
                                <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">Keamanan & Alamat</h3>

                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-1.5">
                                        <label className="ml-1 text-xs font-bold text-slate-600">Password Baru</label>
                                        <div className="relative">
                                            <Lock className="absolute left-3 top-3 size-4 text-slate-400" />
                                            <input type="password" value={data.password} onChange={(e) => setData('password', e.target.value)} placeholder="Kosongkan jika tetap" className="w-full rounded-2xl border-slate-200 bg-white p-2.5 pl-10 text-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" />
                                        </div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="ml-1 text-xs font-bold text-slate-600">Konfirmasi</label>
                                        <div className="relative">
                                            <Lock className="absolute left-3 top-3 size-4 text-slate-400" />
                                            <input type="password" value={data.password_confirmation} onChange={(e) => setData('password_confirmation', e.target.value)} className="w-full rounded-2xl border-slate-200 bg-white p-2.5 pl-10 text-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-1.5">
                                    <label className="ml-1 text-xs font-bold text-slate-600">Alamat Lengkap</label>
                                    <div className="relative">
                                        <MapPin className="absolute left-3 top-3 size-4 text-slate-400" />
                                        <textarea rows="4" value={data.address} onChange={(e) => setData('address', e.target.value)} className="w-full rounded-2xl border-slate-200 bg-white p-2.5 pl-10 text-sm focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10" placeholder="Jl. Raya Siliwangi No. 10..."></textarea>
                                    </div>
