...
function RegisterForm() {
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      {/* Pilihan Role */}
      <div>
        <label className="block text-xs font-black uppercase tracking-widest text-white-600 mb-2 ml-1">
          Daftar Sebagai:
        </label>
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <UserCircle
              className={`size-4 ${data.role === 'customer' ? 'animate-pulse' : ''}`}
            />
          ...
          >
            <Store
              className={`size-4 ${data.role === 'seller' ? 'animate-pulse' : ''}`}
            />
            <span className="text-sm font-bold text-white-300">Mitra</span>
          </button>
        </div>
        {errors.role && (
          <p className="text-red-600 text-[11px] mt-2 ml-1 font-bold animate-shake italic">
      ...
      <div className="space-y-3.5">
        {/* Nama */}
        <div className="group">
          <label className="block text-xs font-bold text-white-600 mb-1 ml-1 group-focus-within:text-teal-600 transition-colors">
            Nama Lengkap
          </label>
          ...
          {errors.name && (
            <p className="text-red-600 text-[10px] mt-1 ml-1 font-bold">
              {errors.name}
            </p>
          )}
        </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full rounded-2xl border p-3 pl-10 text-sm transition-all ${
                errors.email
                  ? 'border-red-300 bg-red-50/30'
                  : 'border-slate-200 focus:border-teal-500 bg-white/50'
              }`}
        ...
        {errors.password && (
          <p className="text-red-600 text-[10px] font-bold ml-1">
            {errors.password}
          </p>
        )}
      </div>
      ...
      <div className="mt-6 text-center border-t border-slate-100 pt-6">
        ...
        <Link
          href={route('login')}
          className="inline-flex w-full items-center justify-center rounded-2xl border border-slate-200 bg-white py-3 text-sm font-bold text-teal-700 transition hover:bg-teal-50 hover:border-teal-300"
        >
          Masuk Akun
        </Link>
      </div>
